...
import { useSongUrl, useDetail, useLyric } from "@/utils/api";
...
export const playerStore = defineStore("player", {
  ...
  actions: {
    ...
    onSliderChange(val: number) {
      ...
      this.geCicurrentTime = this.transTime(val);
    },
    //播放时间拖动中
    ...
    interval() {
      if (this.isPlaying && !this.sliderInput) {
        ...
        this.audio.ontimeupdate = () => {
          ...
          this.geCicurrentTime = this.transTime(a);
    ...
    transTime(time: number) {
      ...
      minute1 = minute < 10 ? "0" + minute.toString() : minute.toString();

      let second = Math.floor(time % 60);

      let second1: string = "";

      second1 = second < 10 ? "0" + second.toString() : second.toString();
